...
```{r load, exercise = TRUE, echo=TRUE}
library(learnNetSci)
```
...
```{r empty, exercise = TRUE, echo=TRUE}
# Let's look into the data
```

Let's transform our edge-list into a network.

```{r create-ppi-intern, exercise = FALSE, echo=FALSE}
gPPI = PPI %>% 
...
```{r create-ppi, exercise = TRUE, echo=TRUE, exercise = TRUE, exercise.lines = 10}
gPPI = PPI %>% 
  select(starts_with("Symbol")) %>%
  filter(Symbol_A != "") %>%
  filter(Symbol_B != "") %>%
...
```{r explore-ppi, exercise = TRUE, exercise.lines = 5}

```

Next, let's check the degree distribution:

```{r degree-distribution, fig.cap="PPI Degree Distribution.", echo=TRUE,  exercise.lines = 10, exercise = TRUE}
dd = degree(gPPI) %>% 
  table() %>% 
  as.data.frame()
...
```{r ppi-hubs, results='markup', exercise.lines = 5, exercise = TRUE}
degree(gPPI) %>% 
  as.data.frame() %>% 
  arrange(desc(.)) %>%
  filter(. > 1000) 
...
```{r empty2, exercise = TRUE, echo=TRUE}
...
```{r ppi-hub, results='markup', exercise.lines = 5, exercise = TRUE}

```

2.  Are those genes connected?

```{r ppi-conn, exercise.lines = 5, exercise = TRUE}
...
```{r emptygda, exercise = TRUE, echo=TRUE}
...
```{r emptygda2, exercise = TRUE, echo=TRUE}

```

```{r GDA-clean, exercise = TRUE, echo=FALSE}
Cleaned_GDA = GDA %>% 
  filter(diseaseType == 'disease') %>%
  mutate(diseaseName = tolower(diseaseName)) %>%
  select(geneSymbol, 
...
```{r emptydistgda, exercise = TRUE, echo=TRUE}
...
```{r, fig.cap= "Gene-Disease degree distribution.", echo=TRUE, exercise = FALSE}
...
```{r gda-topgene, exercise.lines = 5, exercise = TRUE}
...
```{r gda-topdis, exercise.lines = 5, exercise = TRUE}
...
```{r gda-topcl, exercise.lines = 5, exercise = TRUE}
...
```{r, echo=TRUE, results='hide', warning=FALSE, message=FALSE, echo=TRUE, exercise = FALSE}
...
```{r, results='hide', echo=TRUE, exercise = FALSE}
...
```{r components, echo=TRUE, exercise = TRUE}
...
```{r LCC, echo=TRUE, exercise = TRUE, exercise.timelimit = 1000}
...
```{r LCC-deisy, echo=TRUE, exercise = TRUE}

```


```{r SCZplot, echo=TRUE, exercise = TRUE}
gScz 
...
```{r LCC-autism, exercise = TRUE, exercise.lines = 25, exercise.timelimit = 3000}
...
```{r LCC-BD, exercise = TRUE, exercise.lines = 25}

```

- Major Depressive Disorder; 

```{r LCC-MDD, exercise = TRUE, exercise.lines = 25}

```

-   Rheumatoid Arthritis;

```{r LCC-RA, exercise = TRUE, exercise.lines = 25}

```

-   Asthma

```{r LCC-asthma, exercise = TRUE, exercise.lines = 25}

```

-   Parkinson Disease

```{r LCC-PD, exercise = TRUE, exercise.lines = 25}

```

-   Endometriosis

```{r LCC-Endometriosis, exercise = TRUE, exercise.lines = 25}
...
```{r euler, echo=TRUE, exercise = TRUE}
euler(Euler_List)
```
...
```{r sepex, echo=TRUE, exercise = TRUE}
sab = separation(gPPI, GDA_Interest)
...
```{r sab, exercise = TRUE, exercise.lines = 25}
...
```{r sab-net, exercise = TRUE, exercise.lines = 25}
...
```{r brain, exercise = FALSE, echo=TRUE, results='markup'}
...
```{r wtoBD, exercise = FALSE, echo=TRUE, results='markup'}
BD = Brain %>% 
...
```{r wto-play, exercise = TRUE}
head(BD)
```


```{r wtoOther, exercise = FALSE, echo=TRUE, results='markup'}
...
```{r wtofilter, exercise = FALSE, echo=TRUE, results='markup'}
BD_cl = BD %>% 
...
```{r Codina, exercise = FALSE, echo=TRUE, results='markup'}
require(CoDiNA)
DiffNet = MakeDiffNet(Data = list(CTR_cl, BD_cl, MDD_cl, SCZ_cl),
                      Code = c("CTR", "BD", "MDD", "SCZ"))
```

```{r Codina-ex, exercise = TRUE, echo=TRUE, results='markup'}
require(CoDiNA)
DiffNet = MakeDiffNet(Data = list(CTR_cl, BD_cl, MDD_cl, SCZ_cl),
                      Code = c("CTR", "BD", "MDD", "SCZ"))
```
...
```{r , exercise = FALSE, echo=TRUE, results='markup'}
DiffNet_cl = DiffNet %>% 
...
```{r , exercise = FALSE, echo=FALSE, results='markup'}
e = net_fig$Edges 
